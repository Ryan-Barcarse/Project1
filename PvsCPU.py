...
def game():    
    ...
    text = Label(screen, text="Welcome to Wordle - Simple edition!").pack()
    """
    Clear function to reset the screen to a blank page
    """
    def clearScreen():
    ...
    """
    Initialize is_word to be False and secret_word to be an empty list
    """
    is_word = False
    secret_word = [""]
    ...
    """
    Crete a function secret, checks if inputed word inside the entry box is a valid 
    5 letter word and is in the nltk library
    """
    def secret():
            word = Input.get()
    ...
    Label(screen, text="Enter the secret 5-letter word:").pack()
    Input = Entry(screen)
    Input.pack()
    is_word = Button(screen, text="Submit", command=secret).pack()
    screen.mainloop() 
